...
Date: 2026-09-10. Status: P0–P4 software implementation and exact-candidate EFI qualification verified; physical/recovery/media phases remain open.
Repository baseline verified before implementation: `origin/main` at `79302b1b1aba2422ed54ce0c96ab2c2226516817`. The current working tree contains the scoped implementation changes and ignored private/tool outputs.
...
- **P3 — software sub-slice implemented:** Added derived USB evidence sessions and metadata-only ACPI evidence contracts. USB completeness requires unique physical labels/logical ports, tested speed and USB-C orientation where applicable; a complete record is generated by the session and carries physical evidence provenance. No collector output or checkbox alone establishes physical qualification.
- **P3 — real reference evidence captured 2026-09-10:** The sanitized T480s inventory, BIOS observations, physical feature checks, ALC257 identity, private DSDT/SSDT set and partial physical USB topology are recorded in `T480S_REFERENCE_EVIDENCE_2026-09-10.md`. USB-A and internal routes are observed; both Type-C connectors passed physical orientation tests but their logical correlation remains unresolved. This evidence is sufficient to begin P4 without claiming a complete USB map or physical macOS acceptance.
- **P4 — software qualification verified 2026-09-10:** The exact captured 20L8 / BIOS 1.62 / Sequoia 15.0 build 24A335 candidate passed the shared configuration gate with partial USB evidence and explicit unresolved USB-C logical correlation. The trusted Linux x86_64 toolchain selected OpenCore 1.0.7, iASL 20260408 and macserial 2.1.8 from the catalog; ten locked release dependencies were acquired and replayed offline. The builder generated a schema-derived EFI with three reviewed AML additions, protected fake-identity test storage/reuse, and real matching `ocvalidate` `VALID` / exit 0. `python3 -m pytest -q` result: **275 passed in 9.88s**.
- **P4 ACPI result:** The exact private capture was validated as 13 tables, disassembled without failure, and retained only in ignored diagnostics. Twelve OEM SSDTs compiled; the OEM DSDT recompile remains a diagnostic-only failure and is not emitted. The generated EFI contains only `SSDT-PLUG-T480S.aml`, `SSDT-PNLF-T480S.aml` and `SSDT-USBX-T480S.aml`; no complete USB map is claimed.
- **Remaining gates:** Real private SMBIOS generation/reuse choice, physical graphics/audio/input/network/USB-C acceptance, exact Recovery acquisition, P6 Textual parity, P7 qualified media adapters, packaging/host-matrix evidence and P8 physical acceptance remain open. The candidate remains `EXPERIMENTAL`; no `SUPPORTED` claim is permitted.

### P4 completion record — 2026-09-10

Files added or changed: trusted toolchain loader/catalog, private identity service, machine-bound ACPI processor, reviewed T480s/Sequoia profile, schema-driven config generator, P4 manifest contracts, USB unresolved-correlation state, orchestrator pass-through, and P4 unit/integration tests. The final build was produced under the ignored workspace output directory from a fresh staging directory; the output was not committed or published.

Exact software results:

- Configuration evaluation: accepted exact target `15.0/24A335`; plan `build_ready=True`; USB evidence `PARTIAL` with two unresolved logical USB-C checks; first-install route `SS01`.
- Toolchain: catalog-qualified Linux x86_64 OpenCore/ocvalidate `1.0.7`, ACPICA iASL `20260408`, macserial `2.1.8`; all selected files passed size, SHA-256 and version checks.
- Dependency/build: 10 release dependencies resolved, downloaded and then replayed offline; manifest binds the plan, artifact lock, schema, profile, ACPI outputs, evidence digest and redacted identity reference.
- Validation: real matching `ocvalidate` returned exit `0`; final status `VALID`.
- Tests: focused P4 tests `8 passed`; full suite `275 passed in 9.88s`; `compileall` and `git diff --check` passed.

P4 does not close Recovery, media writing, firmware modification, or physical acceptance. It does not promote the T480s to `SUPPORTED`.
