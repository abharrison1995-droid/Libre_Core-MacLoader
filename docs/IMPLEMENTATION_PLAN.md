...
| CI and packaging | Windows/Linux Python 3.11/3.14 workflow and wheel/sdist smoke jobs defined; current local verification is 329 tests, 79.22% branch coverage and source mypy clean with `--follow-imports=skip` across 104 files | Local wheel and isolated clean-target package/TUI smoke pass; local sdist build is unavailable because the host's `build` module cannot run as a module, and hosted results remain pending (S01, S13) |
| Workflow and hardware | Shared `WorkflowService` drives CLI and the workflow-scoped Textual TUI for configuration, evidence, review, dependencies, EFI, Recovery and non-destructive media planning; semantic parity, redacted export and safe resume are tested | Legacy standalone CLI utilities are intentionally not claimed as separate TUI screens; hosted distribution, BIOS/installation guidance and physical acceptance remain open (G6-G7) |

Fresh P5-P7 verification on this Linux/Python 3.13 environment: `python3 -m pytest -q --cov=macloader --cov-branch --cov-fail-under=79` = **329 passed, 79.22% branch coverage**; `python3 -m mypy macloader tests --follow-imports=skip` = **clean across 104 source files**; `python3 -m compileall -q macloader tests` and `git diff --check` completed cleanly. The default mypy invocation remains externally blocked by the installed NumPy stub's Python-version syntax before source checking. The exact P4 configuration/build gate accepted Sequoia 15.0/24A335, and the final real `ocvalidate` run returned `VALID` / exit 0. These results establish local software/toolchain qualification, not correctness of untested exact Recovery, physical media, hosted packaging or physical acceptance paths.

No hosted CI, exact Recovery acquisition, USB write or physical installation was performed during this implementation slice. A wheel was built and installed into an isolated target outside the checkout; its CLI/TUI semantic parity smoke passed. The local host lacks the `build` module, so local sdist generation was not claimed; the tracked CI workflow remains the required hosted wheel/sdist evidence. Matching real OpenCore 1.0.7 `ocvalidate` execution is verified for the exact P4 candidate on Linux x86_64. Existing application and documentation changes are preserved; the remaining defects and qualification gates below remain open.
...
|---|---|---|
| [/] S01 — In Progress; packaging | Scope `.gitignore` build-output rules to avoid ignoring `macloader/build/`; include builder sources and required new modules/resources in the reviewed commit. Check all intended source files, not only efi.py. A local wheel built from ignored working files is insufficient evidence. | A local wheel and isolated clean-target install smoke pass for builder, Recovery, removable and workflow modules, including CLI/TUI semantic parity. Local sdist generation is unavailable because the `build` module is not installed; hosted Windows/Linux wheel/sdist evidence remains OPEN. |
...
| ID / priority / owner | Required change | Acceptance evidence and dependency |
|---|---|---|
| [/] S13 — In Progress; release evidence required; CI/docs | Keep README/START_HERE/handoff current; developer setup must install `.[dev]`. CI should collect branch coverage and publish results; establish measured thresholds for critical modules after adding negative tests rather than inventing a current percentage. | The current local candidate records 329 tests and 79.22% branch coverage; `.[dev]` installation and the 79% gate remain enforced in CI. Isolated wheel/clean-target smoke passes, while hosted Windows/Linux runs and local sdist generation remain OPEN. Critical build/cache/recovery/USB failure cases remain tested regardless of aggregate percentage. G0/G6. |
...
- [x] Audit and acquire the selected release set through the pinned resolver/cache, including archive layout, licenses and SHA-256 checks. The final build was replayed with `offline=True`; broader hosted clean-checkout coverage remains open.
...
Progress: P7 software guards and a Windows-first discovery/backend boundary are implemented. The shared writer refuses unbound plans, requires opaque stable identity/re-enumeration, typed expiring confirmation, bounded no-follow source snapshots and complete EFI/Recovery readback. Linux is explicitly disabled until separately advertised. The Windows backend is injectable and has not been hosted or physical-qualified; this is not yet a qualified release adapter.

R7 update 2026-09-11: three independent Luna reviewers found and the implementation repaired fail-open mounted-volume detection, missing post-write invalidation, and raw serial-derived stale-target diagnostics. The repeated swarm and originating security recheck reported no remaining findings. Hosted Windows/Linux, clean sdist, sacrificial USB and physical boot evidence remain open.
