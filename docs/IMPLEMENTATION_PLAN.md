...
| CI and packaging | Windows/Linux Python 3.11/3.14 workflow and wheel/sdist smoke jobs defined; current local verification is 317 tests, 79.34% branch coverage and source mypy clean with `--follow-imports=skip` across 102 files | Local wheel and isolated clean-target package/TUI smoke pass; local sdist build is unavailable because `build` is not installed, and hosted results remain pending (S01, S13) |
