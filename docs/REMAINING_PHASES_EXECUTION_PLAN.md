...
- Accepted verification baseline before P5: 277 tests, 79.78% branch coverage, mypy clean across 93 files with `--follow-imports=skip`, compileall and diff checks clean. The current P7 candidate has 373 local tests and 81.00% branch coverage.
...
P6 execution record 2026-09-11: the shared `WorkflowService` now drives the CLI and workflow-scoped Textual TUI through configuration creation/resume/import/export/migration, evidence review, exact target/profile/options, acknowledgements, dependency resolution, EFI preview/validation, Recovery discovery/acquisition/cache verification and non-destructive media planning. Workers support cancellation and safe resume; destructive media writing remains disabled. Headless parity tests compare semantic configuration, issue codes and plan/lock results. The current local verification is **373 passed, 81.00% branch coverage**, mypy clean across 104 source files, compileall and diff checks clean. Hosted run `34644459857` passes all four Windows/Linux Python 3.11/3.14 test jobs and both wheel/sdist smoke jobs; the local host still cannot generate an sdist because the `build` module is unavailable.
...
P7 software execution record 2026-09-11: the shared media contract now has immutable plans with source manifests and six required SHA-256 bindings (EFI, Recovery, validation, configuration, toolchain and evidence), opaque public device references, whole-device/partition metadata, immediate re-enumeration and typed confirmations expiring after at most 15 minutes. The Windows adapter parses whole-disk PowerShell records without using drive letters, and its qualified-backend lifecycle requires lock/dismount, bounded write, flush, complete readback and remount. Linux remains explicitly unadvertised. Disposable-image tests cover EFI plus Recovery readback, source mutation, symlink/TOCTOU boundaries, interruption markers, corruption, unsafe device states and adapter ordering.

The local P7 verification result was `python3 -m pytest -q --cov=macloader --cov-branch --cov-fail-under=79`: **373 passed, 81.00% branch coverage**; `python3 -m mypy macloader tests --follow-imports=skip`: **clean across 104 source files**; compileall and `git diff --check`: clean. The earlier isolated wheel/package smoke is recorded in the P6 evidence. Hosted run `34644459857` passed Ubuntu 3.11/3.14 at **80.88%/80.34%** and Windows 3.11/3.14 at **80.18%/79.61%**, with mypy clean across 104 files; both hosted wheel and sdist smoke jobs passed. The local host still cannot run `python3 -m build --wheel --sdist --no-isolation` because `build.__main__` is unavailable. No real Windows device enumeration, Linux advertisement, sacrificial USB write, physical readback or macOS installation has occurred. The physical USB checkpoint remains open and must not be inferred from disposable-image success.

R7 review disposition 2026-09-11: the first three-agent Luna review found two duplicate high findings for fail-open Windows mounted-volume detection and one high finding for missing post-write invalidation, plus one medium finding for raw serial-derived stale-target diagnostics. Repairs added explicit Get-Partition/Get-Volume mount-state discovery with fail-closed parsing, mandatory backend invalidation before remount on write/flush/cancel/readback failure, and opaque public device references in diagnostics. The repeated three-agent review reported no remaining findings; the originating security reviewer’s localized recheck reported no findings; the workflow reviewer’s localized recheck resolved the hosted-evidence documentation finding after run `34644459857` passed. Physical exit evidence remains open.
